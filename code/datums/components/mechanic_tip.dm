...
/datum/mechanic_tip/proc/generate_tip(mob/inspector, atom/inspected)
	return EMBED_TIP(get_description(inspector, inspected))
...
/datum/component/mechanic_desc/Initialize(list/datum/mechanic_tip/tips_to_add, datum/callback/_can_show)
	...
	RegisterSignal(parent, COMSIG_PARENT_EXAMINE, PROC_REF(show_tips))
	RegisterSignal(parent, COMSIG_TIPS_REMOVE, PROC_REF(remove_tips))
...
/datum/component/mechanic_desc/proc/show_tips(datum/source, mob/user, list/examine_text)
	if(can_show && !can_show.Invoke(source, user))
		return

	for(var/tip_name in tips)
		var/datum/mechanic_tip/tip = tips[tip_name]
		examine_text += tip.get_tip(user, source)
