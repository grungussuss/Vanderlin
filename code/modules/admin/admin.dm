...
/datum/admins/proc/show_player_panel_next(mob/M, clicked_flag = null)
	...
	body += "#skills-section, #languages-section, #stats-section { display: none; border: 1px solid black; padding: 10px; width: 100%; box-sizing: border-box; max-width: 100%; overflow-x: hidden; word-wrap: break-word; }"
	body += "#right { flex: 1; border-left: 2px solid black; padding-left: 10px; max-height: 500px; overflow-y: auto; width: 250px; min-width: 250px; box-sizing: border-box;}"
	body += "#right-header { display: flex; justify-content: space-around; padding: 5px; border-bottom: 2px solid black; position: sticky; top: 0; z-index: 10; }"
	body += "#right-header button { flex: 1; margin: 2px; padding: 5px; cursor: pointer; font-weight: bold; border: none; border-radius: 5px; }"
	body += "#right-header button:hover;}"
	...
	var/datum/browser/popup = new (usr, REF(M), 800, 700)
